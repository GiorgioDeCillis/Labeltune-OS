...
export async function getTeamDataForProject(projectId: string) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    ...
    if (profilesError) {
        console.error('getTeamDataForProject: profilesError:', profilesError);
    }
    ...
    if (assignmentsError) {
        console.error('getTeamDataForProject: assignmentsError:', assignmentsError);
    }
    ...
    const team = (allProfiles || [])
        ...
        .map((u: any) => {
            ...
            const status = assignmentData?.status || 'inactive';
